...
class BxTableDef < ActiveRecord::Base
  ...
  has_many :column_defs, :class_name => "BxColumnDef", :foreign_key => :table_def_id, :order => :position
  has_many :index_defs, :class_name => "BxIndexDef", :foreign_key => :table_def_id, :order => :physical_name, :include => :columns
