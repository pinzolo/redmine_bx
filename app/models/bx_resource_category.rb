...
class BxResourceCategory < ActiveRecord::Base
  ...
  has_many :resources, :class_name => "BxResourceNode", :foreign_key => :category_id,
                       :order => :default_path, :conditions => Proc.new { ["parent_id = ?", 0] }
  has_many :all_resources, :class_name => "BxResourceNode", :foreign_key => :category_id,
                           :order => :default_path, :include => [:parent, :values]
